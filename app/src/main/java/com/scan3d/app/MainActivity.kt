...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            server.start()
            DebugLog.log(DebugLog.Tag.SERVER, "NanoHTTPd porta 3001")
        } catch (e: Exception) {
    ...
    private fun checkARCoreDirectly() {
        try {
            ...
            DebugLog.log(DebugLog.Tag.ARCORE, "ARCore: $availability")
            if (availability.isSupported) {
                try { val s = Session(this); s.close()
                    DebugLog.log(DebugLog.Tag.ARCORE, "✓ Session OK") }
                catch (e: UnavailableException) { DebugLog.e(DebugLog.Tag.ARCORE, "Session: ${e.message}") }
                catch (e: SecurityException)    { DebugLog.e(DebugLog.Tag.ARCORE, "Permissão negada") }
            } else { DebugLog.e(DebugLog.Tag.ARCORE, "✕ ARCore não suportado") }
        } catch (e: Exception) { DebugLog.e(DebugLog.Tag.ARCORE, "Erro: ${e.message}") }
    }
    ...
    inner class Scan3DBridge {
        ...
        @JavascriptInterface
        fun startARScan() {
            DebugLog.log(DebugLog.Tag.ARCORE, "startARScan()")
            handler.post {
                try {
        ...
        fun isARAvailable(): Boolean {
            return try {
                ArCoreApk.getInstance().checkAvailability(this@MainActivity).isSupported
        ...
        // ── URLs: fonte única de verdade compartilhada Kotlin ↔ JS ──

        @JavascriptInterface
        fun isConfigured(): Boolean = ConfigStore.isConfigured(this@MainActivity)

        @JavascriptInterface
        fun getApiUrl(): String = ConfigStore.getApiUrl(this@MainActivity)

        @JavascriptInterface
        fun setApiUrl(url: String) {
            ConfigStore.setApiUrl(this@MainActivity, url)
            DebugLog.log(DebugLog.Tag.SYS, "apiUrl salva: $url")
        }

        @JavascriptInterface
        fun getColmapUrl(): String = ConfigStore.getColmapUrl(this@MainActivity)

        @JavascriptInterface
        fun setColmapUrl(url: String) {
            ConfigStore.setColmapUrl(this@MainActivity, url)
            DebugLog.log(DebugLog.Tag.SYS, "colmapUrl salva: $url")
        }

        @JavascriptInterface
        fun markConfigured() {
            ConfigStore.markConfigured(this@MainActivity)
            DebugLog.log(DebugLog.Tag.SYS, "App marcado como configurado")
        }

        @JavascriptInterface
        fun resetConfig() {
            ConfigStore.reset(this@MainActivity)
            DebugLog.log(DebugLog.Tag.SYS, "Config resetada — reload")
        ...
        when (requestCode) {
            REQ_AR -> {
                if (resultCode == RESULT_OK) {
                    val zipPath = ScanBuffer.take()
                    if (zipPath == null) {
                        handler.post {
                            webView.evaluateJavascript(
                                "window.__onUploadError && window.__onUploadError('nenhuma foto capturada')", null)
                        }
                    } else {
                        handler.post {
                            webView.evaluateJavascript("window.__onUploadStart && window.__onUploadStart()", null)
                        }
                        val zipFile = File(zipPath)
                        ScanUploader.uploadZip(this, zipFile) { jobId, error ->
                            handler.post {
                                if (jobId != null) {
                                    DebugLog.log(DebugLog.Tag.SERVER, "Upload ok, job_id=$jobId")
                                    webView.evaluateJavascript(
                                        "window.__onUploadDone && window.__onUploadDone('$jobId')", null)
                                } else {
                                    DebugLog.e(DebugLog.Tag.SERVER, "Upload falhou: $error")
                                    webView.evaluateJavascript(
                                        "window.__onUploadError && window.__onUploadError(${JSONObject.quote(error ?: "erro")})", null)
                                }
                                zipFile.delete()
                            }
                        }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == REQ_CAMERA) {
            ...
            if (granted) { pendingPermissionRequest?.grant(pendingPermissionRequest!!.resources); checkARCoreDirectly() }
    ...
    private fun showLogDialog() {
        ...
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setBackgroundColor(0xFF0a0a0f.toInt()) }
        val header = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL
            setBackgroundColor(0xFF111118.toInt()); setPadding(dp(12), dp(8), dp(12), dp(8))
        }
        header.addView(TextView(this).apply {
            text = "◈ GÊNESIS LOG"; textSize = 13f; typeface = Typeface.DEFAULT_BOLD
        ...
        btnCopy.setOnClickListener {
            ...
            cm.setPrimaryClip(ClipData.newPlainText("genesis_log", DebugLog.dump()))
            Toast.makeText(this, "✓ Copiado", Toast.LENGTH_SHORT).show()
        }
        val btnReset = Button(this).apply {
            text = "RESET"; textSize = 10f; stateListAnimator = null
            setBackgroundColor(0xFFff4444.toInt()); setTextColor(0xFFffffff.toInt())
            setPadding(dp(10), 0, dp(10), 0)
        }
        btnReset.setOnClickListener {
            ConfigStore.reset(this)
            Toast.makeText(this, "Config resetada — reiniciando", Toast.LENGTH_SHORT).show()
            dialog.dismiss()
            webView.reload()
        }
        ...
        header.addView(btnCopy,  LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, dp(34)).apply { setMargins(0,0,dp(6),0) })
        header.addView(btnReset, LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, dp(34)).apply { setMargins(0,0,dp(6),0) })
        header.addView(btnClose, LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, dp(34)))

        val scroll = ScrollView(this)
        val ll = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(8), dp(4), dp(8), dp(4)) }
    ...
    override fun onDestroy() { instance = null; server.stop(); webView.destroy(); super.onDestroy() }
